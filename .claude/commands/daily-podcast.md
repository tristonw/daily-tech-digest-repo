...
```
（采集机器人每 3 小时会往 main 推送，必须先同步，否则最后 push 会被拒。）
...
然后读取 `data/raw/<今天日期>-digest.prompt.txt`（日期 = `python -c "from src import config;print(config.today_str())"`，即北京日期），里面是当天真实采集到的 GitHub / Hacker News / RSS 条目。
...
- 在 front-matter 之后、正文之前，写一个**大纲注释块**（供 shownotes 用），格式：
  ```
  <!-- outline:
  - 开场与今日看点
  - 话题一：……
...
> 关键新闻链接会由发布流程自动从 digest 抽取，无需手写；但请确保 digest（第3步）里每个重要项目/新闻都带了 Markdown 链接。
...
git commit -m "Daily episode <日期>"
git pull --rebase origin main   # 关键：先 rebase，避免被采集机器人的并发提交顶掉
git push origin main
```
（**不要**提交 mp3、site/、data/piper_models、g2pW）
**务必确认 push 成功**（输出里有 `-> main`）。若 push 仍被拒，再 `git pull --rebase origin main` 后重试。这一步失败 = 今天白跑，必须重视。
